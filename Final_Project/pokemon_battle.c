...
char updateHealth(int damage, int recoil, char side) {
	...
	if(side == 'A') {
		while(damage > 0) {
			lcd_draw_rectangle(100 + ALLY_POKEMON_HEALTH, 120- ALLY_POKEMON_HEALTH, 280, 15, LCD_COLOR_WHITE);
			ALLY_POKEMON_HEALTH--;
			damage--;
			for(j = 0; j < 100000; j++){
			}
		...
		while(damage > 0) {
			lcd_draw_rectangle(100 + ENEMY_POKEMON_HEALTH, 120- ENEMY_POKEMON_HEALTH, 280, 15, LCD_COLOR_WHITE);
